...
with st.container(border=True):
    ...
    with cols1[1]:
        ...
        city_pool = avail_df_merge[avail_df_merge["cluster"].astype(str).isin(area_selected)]["city"].dropna().astype(str).unique().tolist()
        city_pick = st.multiselect("Choose City", ["All"] + sorted(city_pool), default=["All"])
        if "All" in city_pick:
            city_pick = city_pool

    brand_pool = avail_df_merge[
        (avail_df_merge["cluster"].astype(str).isin(area_selected))&
        (avail_df_merge["city"].astype(str).isin(city_pick if isinstance(city_pick,list) else [city_pick]))&
...
if button:
    ...
    for nm in names_scope:
        ...
        for i in clusters:
            ...
            if col in avail_df_merge.columns:
                ...
                if not t.empty:
                    t["cluster_labels"] = int(i)
                    pick_avail_lst.append(t)
    ...
    if not pick_avail_lst:
        ...
        pick_avail = pick_avail[pick_avail["cluster"].astype(str).isin(area_selected)]
        ...
        if dealer_rec.empty:
            ...
            cv = sum_df[sum_df.sales_name.isin(names_scope)][["cluster","sales_name"]].groupby("cluster").count().reset_index().rename(columns={"sales_name":"count_visit"})
            cluster_center = pd.merge(cluster_center, cv, on="cluster", how="left")
            ...
            init_lon = float(pd.to_numeric(dealer_rec["longitude"], errors="coerce").dropna().mean()) if pd.to_numeric(dealer_rec["longitude"], errors="coerce").notna().any() else 106.8456
            init_lat = float(pd.to_numeric(dealer_rec["latitude"], errors="coerce").dropna().mean()) if pd.to_numeric(dealer_rec["latitude"], errors="coerce").notna().any() else -6.2088
            st.pydeck_chart(
                pdk.Deck(
                    map_style=None,
            ...
            def some_output(area):
                ...
                if not df_output.empty:
                    ...
                    ) if not pot_df_output.empty else None
                else:
                    fig = None
                    fig1 = None
            ...
            tab_labels = cluster_center["word_pick"].dropna().astype(str).unique().tolist()
            tab_labels.sort()
