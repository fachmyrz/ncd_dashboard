...
name_options = ['A. Sofyan','Nova Handoyo','Heriyanto','Aditya rifat','Nova Handoyo','Muhammad Achlan','Rudy Setya wibowo','Samin Jaya']
st.title("Filter for Recommendation")

with st.container():
    name = st.selectbox("BDE Name ", name_options)
    cols1 = st.columns(2)
    with cols1[0]:
    ...
    with cols1[1]:
        ...
        if not avail_df_merge.empty:
            if name in sales_jabo:
                cities = avail_df_merge[avail_df_merge.get('sales_name') == name]['city'].dropna().unique().tolist()
                jabodetabek_cities = [c for c in cities if c in jabodetabek]
                city_options = jabodetabek_cities if len(jabodetabek_cities) > 0 else sorted(list(set(cities)))
            else:
                cities = avail_df_merge[avail_df_merge.get('sales_name') == name]['city'].dropna().unique().tolist()
                regional_cities = [c for c in cities if c not in jabodetabek]
                city_options = regional_cities if len(regional_cities) > 0 else sorted(list(set(cities)))
        ...
        if "All" in city_pick:
            ...
            city_filter = city_pick

    if not avail_df_merge.empty:
        df_brand_base = avail_df_merge[avail_df_merge.get('sales_name') == name].copy()
        if city_filter is not None:
            df_brand_base = df_brand_base[df_brand_base.get('city').isin(city_filter)]
        if len(penetrated) > 0:
            df_brand_base = df_brand_base[df_brand_base.get('tag').isin(penetrated)]
        if len(potential) > 0:
            df_brand_base = df_brand_base[df_brand_base.get('availability').isin(potential)]
        brand_choose = sorted(df_brand_base['brand'].dropna().unique().tolist())
...
if button and name != "" and penetrated != "" and potential != "" and 'city_pick' in locals():
    ...
    if name in sales_jabo:
        if 'cluster' in pick_avail.columns:
            pick_avail = pick_avail[pick_avail['cluster'] == 'Jabodetabek'] if pick_avail['cluster'].dtype == object else pick_avail
    else:
        if 'cluster' in pick_avail.columns:
            pick_avail = pick_avail[pick_avail['cluster'] != 'Jabodetabek'] if pick_avail['cluster'].dtype == object else pick_avail

    if city_filter is not None:
        pick_avail = pick_avail[pick_avail.get('city').isin(city_filter)] if not pick_avail.empty else pick_avail
    if brand_filter is not None:
        pick_avail = pick_avail[pick_avail.get('brand').isin(brand_filter)] if not pick_avail.empty else pick_avail
    if len(penetrated) > 0:
        pick_avail = pick_avail[pick_avail.get('tag').isin(penetrated)] if not pick_avail.empty else pick_avail
    if len(potential) > 0:
        pick_avail = pick_avail[pick_avail.get('availability').isin(potential)] if not pick_avail.empty else pick_avail

    dealer_rec = pick_avail.copy() if not pick_avail.empty else pd.DataFrame()
