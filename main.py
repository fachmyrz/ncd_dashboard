...
sales_jabo = ['A. Sofyan','Nova Handoyo','Heriyanto','Aditya rifat','Riski Amrullah Zulkarnain','Rudy Setya Wibowo']
...
if button and name != "" and penetrated != "" and potential != "" and city_pick != "":
    ...
    for i in range(len(sum_df[sum_df.sales_name == name].cluster.unique())):
        temp_pick = avail_df_merge[(avail_df_merge.get(f'dist_center_{i}').notna())&(avail_df_merge[f'dist_center_{i}'] <= radius)&(avail_df_merge.sales_name == name)]
        temp_pick = temp_pick.copy()
        temp_pick['cluster_labels'] = i
        pick_avail_lst.append(temp_pick)
    pick_avail = pd.concat(pick_avail_lst)
    pick_avail.drop(columns=[f'dist_center_{i}' for i in range(len(sum_df.cluster.unique()))],inplace=True,errors='ignore')
